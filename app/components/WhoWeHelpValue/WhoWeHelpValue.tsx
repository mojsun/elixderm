...
import styles from './WhoWeHelpValue.module.css'
import { WhoWeHelp } from '@/types/WhoWeHelp'

interface WhoWeHelpValueProps {
  whoWeHelp: WhoWeHelp
}
...
export default function WhoWeHelpValue({ whoWeHelp }: WhoWeHelpValueProps) {
  // Default values that appear on all pages unless overridden in Sanity
  const defaultValues = {
  ...
  const valueData = {
    heading: whoWeHelp?.value?.heading || defaultValues.heading,
    description: whoWeHelp?.value?.description || null,
    images: whoWeHelp?.value?.images && whoWeHelp.value.images.length > 0 
      ? whoWeHelp.value.images 
