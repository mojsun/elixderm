...
import styles from './WhoWeHelpMiddleCTA.module.css'
import { WhoWeHelp } from '@/types/WhoWeHelp'

interface WhoWeHelpMiddleCTAProps {
  whoWeHelp: WhoWeHelp
}

export default function WhoWeHelpMiddleCTA({ whoWeHelp }: WhoWeHelpMiddleCTAProps) {
  const middleCTA = whoWeHelp?.middleCTA || { subheading: '', heading: '', ctaText: '', image: '', imageAlt: '' }

  if (!middleCTA.heading && !middleCTA.image) {
    return null
  }
  ...
  return (
    <section className={styles.middleCta}>
      ...
      <div className={styles.mctaText}>
        {middleCTA.subheading && <p>{middleCTA.subheading}</p>}
        <h2>{middleCTA.heading || 'Ready to get started?'}</h2>

        <Link href="/contact-us">
          <button>
            <span>{middleCTA.ctaText || 'Get Started Today'}</span>
